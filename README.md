...
版本 **1.13.0**。Seedance 2.5 视频 Prompt 生产 Skill：表演外化与情绪提示词、导演与分镜（含导演语法：景别 / 运镜 / 动线 / 剪辑 / 决策载体）、时长与节奏（两条时间轨 + 变化轨）、参考资产计划、Prompt 编译、连续性与质量检查、成片实测。调用：`/film-director` 或在对话中描述任务（拆分镜、转 Prompt、表演测试、改成片）。

1.13.0 接 film-creative 4.0.0 的剧本页：事件轨地点写"同上"（及同前 / 连续 / 接上 / 紧接开头）沿用上一行的场地，括号子空间照旧并入（修复：Offset EP01 s06 原来被列成 3 处场地，实为 1 处）；正文前新增的设计卡只读"静音测试"一格，`scene_track.py` 有则列出，S5 §5.1b 把它当视觉重点与峰值事件的候选——告知，不锁定，不新增必填项，不代替交付变化、变化轨与 W24 / W30–W33；没有设计卡的旧稿照旧。保护区只加 stage-5 一行，哈希重设。**工程完成；效果未验证**。
