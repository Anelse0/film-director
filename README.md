...
版本 **1.5.1**。Seedance 2.5 视频 Prompt 生产 Skill：表演外化与情绪提示词、导演与分镜（含导演语法：景别 / 运镜 / 动线 / 剪辑 / 决策载体）、时长与节奏（两条时间轨 + 变化轨）、参考资产计划、Prompt 编译、连续性与质量检查、成片实测。调用：`/film-director` 或在对话中描述任务（拆分镜、转 Prompt、表演测试、改成片）。
...
| `scripts/scene_track.py` | 剧本场面轨 / 事件轨的参考清单（场地 / 场景 / 段 / 估时）+ 分镜场级合计与比值（≥1.2 提醒）+ 方案"与场面轨的差异"行 |
