...
版本 **1.13.1**。Seedance 2.5 视频 Prompt 生产 Skill：表演外化与情绪提示词、导演与分镜（含导演语法：景别 / 运镜 / 动线 / 剪辑 / 决策载体）、时长与节奏（两条时间轨 + 变化轨）、参考资产计划、Prompt 编译、连续性与质量检查、成片实测。调用：`/film-director` 或在对话中描述任务（拆分镜、转 Prompt、表演测试、改成片）。

1.13.0 接 film-creative 4.0.0 的剧本页：事件轨地点写"同上"（及同前 / 连续 / 接上 / 紧接开头）沿用上一行的场地，括号子空间照旧并入（修复：Offset EP01 s06 原来被列成 3 处场地，实为 1 处）；正文前新增的设计卡只读"静音测试"一格，`scene_track.py` 有则列出，S5 §5.1b 把它当视觉重点与峰值事件的候选——告知，不锁定，不新增必填项，不代替交付变化、变化轨与 W24 / W30–W33；没有设计卡的旧稿照旧。保护区只加 stage-5 一行，哈希重设。**工程完成；效果未验证**。1.13.1 起地点栏空着的行同样沿用上一行、照常算一行（原来整行跳过，`--plan` 会误报"事件轨没有这一行"）。

1.12.0 用户节奏基准写进 skill：对话档默认按用户基准校验（镜长 2–4 s、开头 / 结尾无声各 ≤2 s、句间不留空），违反报 ERROR——R01 单镜 >4 s、R02 首尾无声 >2 s、R03 句间无声 >2 s、R04 给对话场声明表演档；不能在 QA 写理由保留，只有 E 层 `长镜理由`（脚本按类型核对）与 `基准豁免`（用户指定）两条路；项目可在 `rhythm-profile.md` / `production-profile.md`（表头 `节奏基准项`）调整，放宽须写明用户定。W28 另查一镜多句；S5 排完时间线先逐镜核对，S7 用户说慢就整条重核（§7.5）。`scripts/rhythm_baseline.py`。触发：Offset EP01 s04（2/2/2/6/2/6 s，校验 0 error）。**工程完成；效果未验证**。
...
1.9.0 摄影机语言分四层：摄影机身份（观众站在哪里，按场定）/ 支撑 / 运动 / 质感（`director-grammar.md` §2.5），附"资料说适合、但 Seedance 上有风险"的七类风险表（依据只取能力表与 THE ORDER 实测）；更正希区柯克变焦的方向（后拉 + 推焦时背景变大压近）。
...
| `scripts/validate_prompt.py` | S6 之后必跑；R01–R04 / W38 用户节奏基准（`rhythm_baseline.py`，ERROR）、W22–W29 时间轨、W30–W33 变化轨（`rhythm_checks.py` / `variation_checks.py`）、W34–W35 运镜库（`camera_checks.py`）、E23 / W36 / W37 衣着（`wardrobe_checks.py`） |
