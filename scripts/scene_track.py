#!/usr/bin/env python3
"""场面轨：剧本的场地 / 场景清单作参考，分镜的场级合计与之对照（film-director 1.5.0）。
...
场面轨（film-creative 3.6+ 剧本页正文前的"## 场面轨"表：段 / 地点（子空间）/ 主要活动 / 时间 / 谁在画面里 /
观众这段新看见什么 / 锚句 / 估时）对本 skill 是**告知，不是锁定**：分镜可以重分段、换子空间、换活动或地点，
只要在场级方案里写一行"与场面轨的差异：…"（没改写"无"）。台词仍是锁定输入。

- 参考清单：本场有哪些场地 / 场景、几段、有几次时间跳、剧本总估时（场面轨"总估时"或各段估时之和；
  没有场面轨的旧稿回退到节拍表"总窗口 ≈ N s"）。
...
- 差异行：剧本有场面轨、又给了 --plan 时，方案里必须有"与场面轨的差异"一行。
...
DIFF_LINE = re.compile(r'与场面轨的差异\s*[:：]?\s*(.*)')
...
def read_track(text):
    """返回 {'rows', 'total', 'source'}；无场面轨时 rows 为空、total 回退到节拍表总窗口。"""
    rows, total, source = [], None, None
    ...
    if m:
        ...
        if table:
            ...
            idx = {k: next((i for i, h in enumerate(header) if w in h), None) for k, w in COLS}
            for r in table[1:]:
                cells = _cells(r)
...
def run(script, clips=(), plan=None):
    ...
    res = {'script': str(script), 'segments': track['rows'], 'places': places, 'jumps': jumps,
    ...
    if plan:
        ...
        if track['rows'] and res['plan_diff'] is None:
            res['problems'].append('剧本有场面轨，方案里没有"与场面轨的差异：…"一行（没改写"无"）')
    return res
...
def render(res):
    ...
    if res['segments']:
        ...
        out.append(f"{name} 场面轨（参考，不锁定）：{len(res['segments'])} 段，场地 / 场景 {len(res['places'])} 处——{places}"
        ...
        out.append(f'{name}：没有场面轨（旧稿或非 film-creative 来源），按剧本正文与场景标题读场地。')
    ...
    if res['plan_diff'] is not None:
        out.append(f"与场面轨的差异：{res['plan_diff']}")
...
def main(argv=None):
    ...
    ap.add_argument('--plan', help='场级方案（核"与场面轨的差异"一行）')
