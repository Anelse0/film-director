...
修复导演输出 Prompt「【】与描述不换行、挤成一堵字墙」的问题。**根因**：默认核心路径模板 **T2**（`prompt-templates.md`）把【素材绑定】的多条绑定、【整体情绪弧线】的多角色都用 `；` 写成**单行**（与 T1／stage-6 §6.1 的多行块不一致），且顶部没有明确的排版/换行规则——模型会照抄成不换行的长行。

- `templates/prompt-templates.md`（受保护，重算哈希）：①顶部新增「排版」规则——每个【】段落另起一行、【素材绑定】每条一行、【整体情绪弧线】每角色一行、【分镜时间线】每镜一行，骨架里为省版面用 `；` 连写的示例落到 Prompt 时按此拆行；②T2 模板的【素材绑定】改为每条一行、【整体情绪弧线】改为标题一行＋每角色一行、【分镜时间线】示例给出逐镜行，与 T1 一致。
- `SKILL.md` 默认输出契约补一句同样的排版要求（非受保护）。
- 纯排版/措辞修正，无代码、无新校验码；100 unittest + shell 回归通过。
...
- **用途尾标**：每条绑定行以 `-形象参考 / -服饰参考 / -场景参考 / -音色参考 / -动作参考` 收尾（§4.1 四类，本 skill 把"形象"细分出"服饰"）。
- **唯一 + 全程锁定**：人物形象/服饰各只一张，尾标写"形象唯一参考 / 服饰唯一参考"（呼应 §4.2"不建议…易混淆或重复角色"，防脸从服饰图取），锁定外观再加"，全程锁定"（本地 `[推论]`；官方"锁定"只用于任务类型）。
- **正文引用拆图人物**：用官方 @ 语法把人与服饰绑一起——`{角色}（@图片{形象号} 身穿@图{服饰号}）`（如 `Mack（@图片7 身穿@图8）`）；@图片N 是官方允许的指代写法（§4.2）。
- 人物形象行用精简的"（面部/发型/体型）"标范围，不再写"只参考…不参考…"；场景保留"不参考人物"。
- 改 stage-6 §6.2、prompt-templates（T2/T3/T4/T5＋起始状态/贯穿要求示例＋英文 REFERENCES）。受保护区重算这两个哈希；validate_prompt、W24、情绪/时间线逻辑一行未动；用途尾标/唯一/全程锁定/@图片 均不触发校验（@图片N 照常计入素材引用）。
- 测试：99 unittest + shell + CI 预检通过。

## 1.4.1 — 2026-09-09

措辞修正（无代码/行为变化）：1.4.0 把“每角色两图”的稳定度写成“主体图数=角色数×2，>8 需抽卡”，**把图片数当成了主体数**。官方 §3 的“1–8 个主体更稳、9–12 需抽卡”数的是**主体（角色）数**、不是图片数；“≤5 个主体单/多视图均可”明确允许每主体多张图。所以人物形象／服饰硬拆两条只增图、不增主体，**不因图变多触发抽卡**；真正门槛是**角色数 >8**，图片另有 30 张硬上限。

- 改 stage-6 §6.2 图数行、stage-5b 服饰图行、references/production-profile 素材绑定约定：一律改为按主体（角色）数计稳定度、图数只受 30 张硬上限约束。
- 受保护区重算 stage-6、stage-5b 两个哈希；validate_prompt 及情绪/时间线逻辑一行未动。
- 测试：99 unittest + shell + CI 预检通过。
