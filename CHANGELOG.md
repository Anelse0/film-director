...
交接物回到场景稿本身：SKILL 输入条与 `handoff-contract.md` §一 / §五 把"台词设计表必附"改为"场景稿必附（台词锁：逐字台词 + 动作行 + 头部来源注），设计表按需"；〔说法〕〔情绪〕从场景稿的台词与动作行映射，场景稿里没有的句子不进 Prompt；无场景稿仍是剧本层缺口交回创作侧，生产侧不补写台词（用户明确锁定糙台词除外）。对应 film-creative 2.1.2。受保护核心未变。

## 2.1.0 — 2026-09-15

新增非保护参考 `references/micro-expression-tiers.md`：眼眉骤变类表情（瞳孔骤变、眼睛突然睁大、眼白露出、眉一闪；次级：眨眼加快、二次吞咽、颜色走上脸、嘴角一抽、"眼睛不眨"硬指令）的 A / B 分类、严 / 中 / 宽三档（按紧景别与每 clip 上限）、S5 定档写进头部、原文不豁免的改写法（骤变 → 保持、闪过 → 慢变、hinge 不可改则改档或改标 free）。它把 `performance-grammar.md` §4"亚秒级闪过不写"落到可判定，依据为 2026-09-15 用户实测单样本 `[推论·单样本]`，已登记 `validation-log.md` 待验证队列；升级进 §4 需 ≥3 条 clip 成片对照。SKILL.md S4 读取行、停靠规则（S5 同时定档）、快速路由各加一行；README 文件地图加一行。**词表、阈值与 FAIL / WARN 留在项目 / 工作区闸门**（例：工作区 `tools/scene_gate.py`），不进 `validate_prompt.py`；受保护核心 29 文件哈希未变。

## 2.0.1 — 2026-09-15

只改协作边界，不动受保护的生产核心（哈希清单 29 文件未变）。台词设计表由"对白场进入生产时默认附、无表则生产侧补写"改为"**必附**；无表 = 剧本层缺口，登记交回 film-creative 出表（生产版：每句一行、逐字、来源标注）；只有用户明确锁定糙台词时才由生产侧补〔说法〕并标注"。触发：2026-09-15 一条对白场 Prompt 的台词在本 skill 内被顺手补写、未经创作侧设计，成片与文本"太平"（有台词镜 69%、每镜子标签 1.2，同项目金样 92% / 0.3）；契约里"默认附"与创作侧"按需"之间的缝隙就是入口。交接契约另记录项目层可自建落盘前闸门（如工作区 `tools/scene_gate.py`），不进入本 skill 的校验脚本。对应 film-creative 2.1.0 的设计表生产版。
