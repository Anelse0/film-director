...
```
【素材绑定】视频1 = 白模参考视频，只参考运镜、镜头节奏、景别变化、主体运动轨迹与镜头调度，严格保持镜头顺序，不参考画面内容；图1 = {角色}外观…；图2…图N = 各阶段关键帧参考。
【总述】…
【画面基调】…
【分镜时间线】
镜头1（0-{a}s，参考 图片2）：…
镜头2（{a}-{b}s，参考 图片3）：…
【贯穿要求】所有画面参考对应关键帧，白模视频仅作为运镜、镜头运动和角色动画参考，不参考画面内容；…；不要字幕。
```
...
【分镜时间线】
镜头1（0-{a}s）：…
【贯穿要求】{只写续写段新增或必须保持的项：外观锁 / 光源 / 声音策略与视频1一致}；不要字幕。
```
