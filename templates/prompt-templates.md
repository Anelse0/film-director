...
```
【素材绑定】图1 = {角色A} 人物形象，只参考面部、发型、体型，不参考服装、背景与姿势；图2 = {角色A} {本场}服装造型（{服装/配饰}），只参考服装与配饰，不参考面部、背景与姿势；图3 = {场景}布局与光，不参考图中人物；音频1 = {角色A}音色，不参考内容。图1使用音频1音色。（多角色逐一按"形象＋服饰"两条列清；换装态各加一条服饰参考。本段只列编号＋职责，约定/数量/草稿/来源等注释归 E 层、不写进正文——W24。）
【总述】…
【画面基调】…（与图3光线一致）
【起始状态】图1的{角色A}在画面…
...
```
【素材绑定】视频1 = 动作与运镜参考，只参考{动作/运镜/节奏}，不参考人物、服装与场景；图1 = {角色}人物形象（面部/发型/体型），不参考服装/背景/姿势；图2 = {角色}本场服装造型（{服装}），只参考服装与配饰…
【总述】…
【画面基调】…
【起始状态】…
...
```
以图片1至图片{N}的顺序作为关键帧。
【素材绑定】{逐项列出图1、图2…图N的实际用途，不用范围缩写代替绑定}；图{N+1} = {角色}人物形象（面部/发型/体型）；图{N+2} = {角色}本场服装造型（{服装}）…
【总述】…
【画面基调】…
【分镜时间线】
镜头1（0-{a}s，参考图片1）：…
...
```
【素材绑定】图1 = 故事板分镜（{N}格，线稿），只参考镜头结构、景别、顺序与动作大意，不渲染格线、编号与文字；图2 = {角色}人物形象（面部/发型/体型）；图3 = {角色}本场服装造型（{服装}）…
【总述】{故事梗概一句话}…
【画面基调】…
【起始状态】…
...
```
REFERENCES: Image1 = {Character A} identity (face, hair, build only; not wardrobe, background or pose). Image2 = {A}'s wardrobe for this scene ({outfit}; wardrobe and accessories only; not face or background). Audio1 = {A}'s voice timbre; Image1 uses Audio1's voice.
