...
**不以导演、影片、摄影师名字代替可执行描述；通用技术名可配说明使用**（校验 W13）。透镜只写在文末 E 层参数表的「透镜」行，供多文件校验用。E 层可选节奏字段（`duration-rhythm.md` §六）：`节奏档`（对话 / 表演）、`台词填充率`（默认 0.9；1.0 = 窗口 = 词数 ÷ 语速向上取整）、`动作节拍秒数`、`台词密度`、`台词占比上限`，以及各阈值覆盖项；11 镜以上写「切点理由」行回应 W04。
