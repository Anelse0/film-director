...
```
【素材绑定】图1 = {角色A} 形象（面部/发型/体型） -形象唯一参考，全程锁定；图2 = {角色A} {本场}服装造型（{服装/配饰}） -服饰唯一参考，全程锁定；图3 = {场景}，只参考布局与光、不参考人物 -场景参考，全程锁定；音频1 = {角色A}音色 -音色参考。图1使用音频1音色。（多角色逐一按"形象＋服饰"两条列清；换装态各加一条服饰参考。每行以官方 §4.1 用途收尾；正文引用拆图人物用 {角色A}（@图片1 身穿@图2）；本段只列编号＋主体＋用途，约定/数量/草稿/来源等注释归 E 层——W24。）
【总述】…
【画面基调】…（与图3光线一致）
